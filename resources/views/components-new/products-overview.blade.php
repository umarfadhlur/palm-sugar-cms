          ...
          <div class="service-item h-100 d-flex flex-column">
            ...
            <div class="p-4 flex-grow-1 d-flex flex-column">
              ...
              <div class="row g-2 mt-2 mb-3"
                ...
                @if ($product->origin)
                <div class="col-6">
                  <small style="color: color-mix(in srgb, var(--default-color), transparent 50%); display:block; text-transform:uppercase; font-size:11px; letter-spacing:0.06em;">Origin</small>
                  <strong style="font-size:13px;">{{ $product->origin }}</strong>
                </div>
                @endif
                @if ($product->cupping_score)
                <div class="col-6">
                  <small style="color: color-mix(in srgb, var(--default-color), transparent 50%); display:block; text-transform:uppercase; font-size:11px; letter-spacing:0.06em;">Cupping Score</small>
                  <strong style="font-size:13px;">{{ $product->cupping_score }}</strong>
                </div>
                @endif
                @if ($product->altitude)
                <div class="col-6">
                  <small style="color: color-mix(in srgb, var(--default-color), transparent 50%); display:block; text-transform:uppercase; font-size:11px; letter-spacing:0.06em;">Altitude</small>
                  <strong style="font-size:13px;">{{ $product->altitude }}</strong>
                </div>
                @endif
                @if ($product->capacity)
                <div class="col-6">
                  <small style="color: color-mix(in srgb, var(--default-color), transparent 50%); display:block; text-transform:uppercase; font-size:11px; letter-spacing:0.06em;">Capacity</small>
